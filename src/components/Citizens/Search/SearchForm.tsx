  ...
  switch (variant) {
    default:
      return {
        test: (lines) =>
          lines.length >= 5 &&
          /Dowód osobisty/.test(lines[0]) &&
          /Data urodzenia.+/.test(lines[2]) &&
          /Wzrost.+/.test(lines[3]),
        extract: (lines) => ({
          Name: lines[1].split(' ')[0],
          Surname: lines[1].split(' ')[1],
          BirthDate: lines[2].split(' : ')[1],
          Height: lines[3].split(': ')[1],
        }),
      };
  }
};
...
function SearchForm(props: Props) {
  ...
  const { control, handleSubmit, reset, watch, errors, clearErrors } = useForm<ISearchFormInput>();
  ...
  return (
    ...
    >
      <Controller
        ...
        defaultValue=''
        rules={{
          validate: validateField(disableNameSurname, t('Podaj imię i nazwisko lub nr. telefonu!')),
          pattern: {
            value: /^[^ ][A-z ]+[^ ]$/,
      ...
      <Controller
        ...
        defaultValue=''
        rules={{
          validate: validateField(disablePhoneNumber, t('Podaj imię i nazwisko lub nr. telefonu!')),
          pattern: {
            value: /^[0-9]{3}-[0-9]{4}$/,
