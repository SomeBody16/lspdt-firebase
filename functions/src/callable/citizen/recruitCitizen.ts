...
import {
  makeRegistration,
  IDiscordLogOptions,
} from "../../registry/makeRegistration";
    ...
    const discordLogOptions: IDiscordLogOptions[] = [
      {
        ...
        customMessage: (msg) =>
          msg.addField(
            "Ranga",
            `${officerDoc.get("Rank.Callsign")} | ${officerDoc.get(
              "Rank.Name"
            )}`
          ),
      },
...
export const createNewOfficer = async (
  ...
  console.log("createNewOfficer", Server);
  const email = `${citizenDoc.data()?.Name}.${
  ...
  const userCreateRequest: admin.auth.CreateRequest = {
    ...
    displayName: `${citizenDoc.data()?.Name} ${
      citizenDoc.data()?.Surname
    } | ${Server}`,
    disabled: false,
  };
